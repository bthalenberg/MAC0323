...
static Operand *create_operand(Buffer *b, SymbolTable alias_table, const char **errptr, int i) {
    ...
    if (validate_label(b, s, errptr, alias_table, i, 1)) {
        ...
        if (alias) return operand_create_register(alias->opd->value.reg);
...
static Operand *process_operand(Buffer *b, SymbolTable alias_table, int k, const char **errptr,
                            const Operator *opt, const char *s, Instruction *head) {
...
int parse(const char *s, SymbolTable alias_table, Instruction **instr, const char **errptr) {
    ...
    while (s[i] != '\0' && s[i] != '\n' ) {
        ...
        for (k = 0; k < opdNumber; k++) {
            ...
            opd[k] = process_operand(aux, alias_table, k, errptr, opt, s, *instr);
        }
